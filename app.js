...
/* ═══ STORAGE KEYS ═══ */
const K={s:'gdn-skripsi-v5',k:'gdn-kuliah-v2',w:'gdn-kerja-v2',n:'gdn-notes-v4',sched:'gdn-schedule-v1',sync:'gdn-sync-url'};
...
function cloudSync(){
  ...
  syncTimer=setTimeout(async()=>{
    const data={state,kuliahTasks,kerjaTasks,notes,schedule};
    try{
      await fetch(syncUrl,{method:'POST',body:JSON.stringify(data),headers:{'Content-Type':'text/plain'}});
      $('syncIndicator').textContent='synced just now';
    }catch{$('syncIndicator').textContent='sync failed'}
  },1500);
}
async function cloudLoad(){
  ...
  try{
    const r=await fetch(syncUrl);const d=await r.json();
    if(d&&d.state){state=d.state;SS(K.s,state);kuliahTasks=d.kuliahTasks||[];SS(K.k,kuliahTasks);kerjaTasks=d.kerjaTasks||[];SS(K.w,kerjaTasks);notes=d.notes||{};SS(K.n,notes);schedule=d.schedule||schedule;SS(K.sched,schedule)}
    $('syncIndicator').textContent='synced';
  }catch{$('syncIndicator').textContent='offline mode'}
}
...
$('saveSettings').addEventListener('click',()=>{
  syncUrl=$('syncUrl').value.trim();localStorage.setItem(K.sync,syncUrl);
  $('settingsModal').classList.remove('open');
  if(syncUrl){$('syncStatus').textContent='Saved! Loading data...';cloudLoad().then(()=>{renderAll();$('syncStatus').textContent='Connected!'})}
  else{$('syncStatus').textContent='Cleared — offline mode.'}
});
...
function updateBrief(){
  ...
  if(todayClasses.length)chip(b,todayClasses.length+' class today: '+todayClasses.map(c=>c.name.split(' ')[0]).join(', '),'ok');
  ...
  if(!b.children.length)chip(b,'All clear today!','ok');
}
...
function buildTask(task,cat,container){
  ...
  let dt=task.day||'';if(task.date)dt+=' &middot; '+fmtDate(task.date);
  if(ds&&dl===0)dt+=' <span class="due-badge">TODAY</span>';else if(ds)dt+=' <span class="due-badge">'+dl+'d left</span>';
  art.innerHTML=`<label class="check"><input type="checkbox" ${done?'checked':''}><span>&#10003;</span></label>
    ...
    <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px"><div class="tag">${task.tag||''}</div>${cat!=='skripsi'?'<button class="btn-del" title="Delete">&#10005;</button>':''}</div>
    <div class="note-area${nt?' open':''}"><textarea placeholder="Tulis catatan...">${nt}</textarea></div>`;
  art.querySelector('input').addEventListener('change',e=>{
  ...
  if(cat!=='skripsi')art.querySelector('.btn-del')?.addEventListener('click',()=>{if(!confirm('Hapus?'))return;if(cat==='kuliah'){kuliahTasks=kuliahTasks.filter(t=>t.id!==task.id);save(K.k,kuliahTasks)}else{kerjaTasks=kerjaTasks.filter(t=>t.id!==task.id);save(K.w,kerjaTasks)}renderTab(cat);updateBrief()});
...
function renderSkripsi(){
  ...
  Object.entries(g).forEach(([s,list])=>{const sec=document.createElement('section');sec.className='section';sec.innerHTML=`<div class="section-header"><small>${list[0].phase}</small><h3>${s}</h3></div><div class="timeline"></div>`;const tl=sec.querySelector('.timeline');list.forEach(t=>buildTask(t,'skripsi',tl));c.appendChild(sec)});
...
$('editSchedule').addEventListener('click',()=>{openScheduleEditor()});
function openScheduleEditor(){
  const f=$('scheduleForm');f.innerHTML='';
  schedule.forEach((c,i)=>{
    const r=document.createElement('div');r.className='sched-form-row';
    r.innerHTML=`<select data-i="${i}" data-f="day"><option>Senin</option><option>Selasa</option><option>Rabu</option><option>Kamis</option><option>Jumat</option><option>Sabtu</option><option>Minggu</option></select>
      <input data-i="${i}" data-f="name" value="${c.name}" placeholder="Nama matkul">
      <input data-i="${i}" data-f="time" value="${c.time}" placeholder="18:30 - 20:00">
      <input data-i="${i}" data-f="room" value="${c.room||''}" placeholder="Ruangan" style="max-width:90px">
      <button class="btn-del" data-del="${i}">&#10005;</button>`;
    r.querySelector('select').value=c.day;f.appendChild(r);
  });
  $('scheduleModal').classList.add('open');
}
$('addClassBtn').addEventListener('click',()=>{schedule.push({day:'Senin',name:'',time:'',room:''});openScheduleEditor()});
$('closeSchedule').addEventListener('click',()=>$('scheduleModal').classList.remove('open'));
$('saveSchedule').addEventListener('click',()=>{
  const rows=$('scheduleForm').querySelectorAll('.sched-form-row');
  const newSched=[];
  rows.forEach(r=>{
    const day=r.querySelector('select').value;const name=r.querySelector('[data-f="name"]').value.trim();
    const time=r.querySelector('[data-f="time"]').value.trim();const room=r.querySelector('[data-f="room"]').value.trim();
    if(name)newSched.push({day,name,time,room});
  });
  schedule=newSched;save(K.sched,schedule);renderSchedule();updateBrief();$('scheduleModal').classList.remove('open');
});
$('scheduleForm').addEventListener('click',e=>{if(e.target.dataset.del!==undefined){schedule.splice(+e.target.dataset.del,1);openScheduleEditor()}});
...
function addTask(cat){
  ...
  $(p+'-title').value='';$(p+'-tag').value='';$(p+'-date').value='';renderTab(cat);updateBrief();
}
...
/* ═══ FILTERS ═══ */
document.querySelectorAll('[data-cat="skripsi"]').forEach(btn=>{btn.addEventListener('click',()=>{
  document.querySelectorAll('[data-cat="skripsi"]').forEach(b=>b.classList.remove('active'));btn.classList.add('active');filters.skripsi=btn.dataset.filter;renderSkripsi();
})});
...
function renderAll(){renderSchedule();renderSkripsi();renderCustom('kuliah');renderCustom('kerja');updateBrief()}
...
/* ═══ INIT ═══ */
initJarvis();renderAll();
if(syncUrl)cloudLoad().then(renderAll);
